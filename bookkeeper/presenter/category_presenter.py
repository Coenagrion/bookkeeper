...
class CategoryPresenter:
    ...
    def __init__(self, view, cat_repo: SQLiteRepository, exp_repo: SQLiteRepository):
        ...
        self.view.category_delete_button_clicked(self.handle_category_del_button_clicked)
    ...
    def handle_category_edit_button_clicked(self) -> None:
        select = self.view.get_selected_cat()
        new_name = self.view.get_category_name()
        cat = Category(name=str(new_name), parent=self.cat_repo.get(select).parent, pk=select)
        self.cat_repo.update(cat)
        self.show()

    def handle_category_del_button_clicked(self) -> None:
